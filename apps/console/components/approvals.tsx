...
export async function ApprovalsPage({workspace,identifier,result}:{workspace?:string,identifier?:string,result?:string}) {
  ...
  if(identifier) {
    ...
    if(d) {
      ...
      const routeChange='route_id' in a.payload ? a.payload : null;
      ...
      content=<><Link href={`/approvals?workspace=${selected}`}>All approvals</Link><h2>{a.payload.label}</h2>
        ...
        {routeChange?<><h3>Exact evaluated AI route</h3><pre>{JSON.stringify(routeChange,null,2)}</pre><p>One synthetic route activation; no live provider or business approval.</p></>:policyChange?<><h3>Exact proposed policy</h3><p>Review the current pointer, candidate, typed rules and validity bounds before granting one activation.</p><pre>{JSON.stringify(policyChange,null,2)}</pre></>:<p>Scenario: {'scenario' in a.payload?a.payload.scenario:''}. Every listed target is part of the frozen set. Dependencies: current policy, fake endpoint, available runtime budget and quota.</p>}
